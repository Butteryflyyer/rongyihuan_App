//
//  BindingCardTableViewController.m
//  HaiHeApp
//
...
@interface BindingCardTableViewController ()
@property (nonatomic, retain)NSArray * titleArr;
@property (nonatomic, retain)UITextField * bancardTF;
...
//@property (nonatomic, retain)NSString * zhucetimeStr;
//@property (nonatomic, retain)NSString * phoneStr;
@end

@implementation BindingCardTableViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"绑定银行卡";
    _titleArr = @[@"银行卡号",@"认证金额(元),至少1元"];
    _userid = [UserLoginStatus shareManager].userid;
...
- (RightImageTableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString * cellidentifi = @"bindingcell";
    ...
    if (indexPath.row==0) {
        _bancardTF = cell.titleTextfield;
        _bancardTF.keyboardType = UIKeyboardTypeNumberPad;
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    footerV.frame = CGRectMake(0, 0, tableView.frame.size.width, 300);
    ...
    //duigouBtn.backgroundColor = nav_bgcolor;
    ...
    [footerV addSubview:xieyiBtn];
    
    UIButton * submitBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    _submitBtn = submitBtn;
    submitBtn.frame = CGRectMake(10, 44, footerV.frame.size.width-20, 44);
    ...
    [footerV addSubview:tishiL];
    
    UITextView * textV = [[UITextView alloc] initWithFrame:CGRectMake(10, 140, footerV.frame.size.width-20, 300)];
...
//- (void)checkbankButtonBeTouch:(id)sender{
//    //弹框显示 各个银行限额;
//}
...
- (void)submitButtonBeTouch:(id)sender{
    ...
    NSString * bankcardStr = [_bancardTF.text stringByReplacingOccurrencesOfString:@" " withString:@""];
    NSString * moneyStr = [_moneyTF.text stringByReplacingOccurrencesOfString:@" " withString:@""];
    if ([bankcardStr isEqualToString:@""]){
        [[ShowMessageView shareManager] showMessage:@"银行卡不能为空"];
    }else if ([moneyStr intValue]<1){
        [[ShowMessageView shareManager] showMessage:@"实名认证至少需充值1元"];
    }else{
        ...
        submitBtn.backgroundColor = [UIColor lightGrayColor];
                            [[HaiHeNetBridge sharedManager] rechargeRzAndCzRequestWithUserId:_userid andWithMoney:moneyStr andWithIdCard:_idcardStr andWithRealname:_realnameStr andWithBankCard:bankcardStr andWithOrderid:simOrder WithSuccess:^(NSString *respString, NSDictionary *datadic) {
                                submitBtn.enabled = YES;
                                submitBtn.backgroundColor = nav_bgcolor;
                                if (!respString) {
...
//                    NSString *simOrder = [NSString stringWithFormat:@"%@%@",[dateFormater stringFromDate:[NSDate date]],suiji];
//                    
//                    [[HaiHeNetBridge sharedManager] rechargeRzAndCzRequestWithUserId:_userid andWithMoney:_moneyTF.text andWithIdCard:_idcardStr andWithRealname:_realnameStr andWithBankCard:_bancardTF.text andWithOrderid:simOrder WithSuccess:^(NSString *respString, NSDictionary *datadic) {
//                        if (!respString) {
//                            if ([[datadic objectForKey:@"ddFlag"]isEqualToString:@"1"]) {
//                                
//                                //开始连连支付;
...
- (NSString *)appendRiskStrig{
    ...
    [riskstr appendFormat:@"{\"%@\":\"%@\",",@"user_info_full_name",_realnameStr];
    ...
    [riskstr appendFormat:@"\"%@\":\"%@\",",@"user_info_id_no",_idcardStr];
    ...
    switch (resultCode) {
        case kLLPayResultSuccess:
        {
            ...
            if ([result_pay isEqualToString:@"SUCCESS"])
            {
                ...
                if ([_issetdealpwStr isEqualToString:@"已设置"]) {
                    UIAlertController * alertC = [UIAlertController alertControllerWithTitle:@"提示" message:@"实名认证成功" preferredStyle:UIAlertControllerStyleAlert];
